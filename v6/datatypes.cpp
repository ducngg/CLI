...
//Before | or , or / (be used in CLI statement)
...
//After | or , or / (be used in CLI statement)
...
//Next word (be used in CLI statement)
string Name::_nextWord(string s){
    return s.substr(0,s.find(' '));
}
//The rest part except first word (be used in CLI statement)
...
string Name::vV(){
    ...
    string v[13]={"a","oa","ua","e","ie","uo","oe","ue","uye","i","uy","o","u"};

    if(VIETNAMESE_NAME) return v[rand()%13];
    return s[rand()%17];
}
