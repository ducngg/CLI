...
template<class Student_Type> void List<Student_Type>::show(bool top){
    ...
    //If chosen to show top students
    float best=0;
    if(top)
    {
...
template<class Student_Type> void List<Student_Type>::random(int n, int n_words_in_name){
    ...
    for(int i=0;i<n;i++)
    {
        ...
        int rday=0;     //random day
        int rmonth=0;   //random month
        int ryear=0;    //random year
...
template<class Student_Type> bool List<Student_Type>::forget(int a, int b){
    if(a<0 || b>=count || a>b) return false;
...
void UniversityStudent::doAssignment(){
    for(int i=0;i<N_UNI_SEM;i++)
        for(int j=0;j<UNI_SEM_ASSIGNMENT;j++)
            if(!__sem[i]->Assignments[j].valid)
                __sem[i]->Assignments[j].work();
}   
void UniversityStudent::takeTest(){
    for(int i=0;i<N_UNI_SEM;i++)
        for(int j=0;j<UNI_SEM_TEST;j++)
            if(!__sem[i]->Tests[j].valid)
                __sem[i]->Tests[j].work();
}
void UniversityStudent::takeExam(){
    for(int i=0;i<N_UNI_SEM;i++)
        for(int j=0;j<UNI_SEM_EXAM;j++)
            if(!__sem[i]->Exams[j].valid)
                __sem[i]->Exams[j].work();
}
float UniversityStudent::gpa(){
    float sum=0;
    int n=N_UNI_SEM*(UNI_SEM_ASSIGNMENT+UNI_SEM_TEST+UNI_SEM_EXAM);

    for(int i=0;i<N_UNI_SEM;i++)
    {
        for(int j=0;j<UNI_SEM_ASSIGNMENT;j++) 
            if(__sem[i]->Assignments[j].valid)
                sum+=__sem[i]->Assignments[j].point;
            else
                n--;

        for(int j=0;j<UNI_SEM_TEST;j++)
            if(__sem[i]->Tests[j].valid)
                sum+=__sem[i]->Tests[j].point;
            else
                n--;

        for(int j=0;j<UNI_SEM_EXAM;j++)
            if(__sem[i]->Exams[j].valid)
                sum+=__sem[i]->Exams[j].point;
            else
                n--;
    }
...
void CollegeStudent::doAssignment(){
    for(int i=0;i<N_COL_SEM;i++)
        for(int j=0;j<COL_SEM_ASSIGNMENT;j++)
            if(!__sem[i]->Assignments[j].valid)
                __sem[i]->Assignments[j].work();
}   
void CollegeStudent::takeTest(){
    for(int i=0;i<N_COL_SEM;i++)
        for(int j=0;j<COL_SEM_TEST;j++)
            if(!__sem[i]->Tests[j].valid)
                __sem[i]->Tests[j].work();
}
void CollegeStudent::takeExam(){
    for(int i=0;i<N_COL_SEM;i++)
        for(int j=0;j<COL_SEM_EXAM;j++)
            if(!__sem[i]->Exams[j].valid)
                __sem[i]->Exams[j].work();
}
float CollegeStudent::gpa(){
    float sum=0;
    int n=N_COL_SEM*(COL_SEM_ASSIGNMENT+COL_SEM_TEST+COL_SEM_EXAM);
...
Semester::Semester(int __A, int __T, int __E){
    __nAssignment=__A;
    __nTest=__T;
    __nExam=__E;

    if(__nAssignment)
        Assignments=new Work[__nAssignment];
    if(__nTest)
        Tests=      new Work[__nTest];
    if(__nExam)
        Exams=      new Work[__nExam];
}

Semester::~Semester(){
    if(Assignments) delete[] Assignments;
    if(Tests)       delete[] Tests;
